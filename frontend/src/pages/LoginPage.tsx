      ...
      <div className="w-full max-w-[420px] min-h-screen bg-[#f6f7f3] flex flex-col px-7 md:my-6 md:min-h-[calc(100vh-3rem)] md:rounded-[32px] md:shadow-[0_20px_60px_-25px_rgba(20,40,30,0.35)]">
        {step === "phone" && (
          <div className="flex-1 flex flex-col pt-16 pb-8">
            ...
            <h2 className="font-serif text-[29px] leading-[1.05] mb-2">
              Welcome to My School Space
            </h2>
            <p className="text-[13px] text-muted leading-[1.55]">
              Sign in with the mobile number your school has on record. No
            ...
            <div className="mt-[30px]">
              ...
              <div className="flex items-center bg-white border-[1.5px] border-line rounded-[14px] overflow-hidden">
                <span className="px-3 text-[14px] font-semibold border-r border-line h-12 flex items-center">
                  +91
                </span>
                <input
                  value={phone}
              ...
              {error && (
                <div className="text-[11.5px] text-danger font-semibold mt-2">
                  {error}
                </div>
              )}
            </div>
            ...
            <div className="flex gap-[9px] items-start bg-[#eef3ee] border border-[#e0e7e0] rounded-[14px] px-[13px] py-3">
              ...
              <div className="text-[11px] text-[#3a4a41] leading-[1.5]">
                There's nothing to sign up for. Your school registers your
                number — parents are added by the class teacher, staff by the
                admin.
              </div>
            </div>
          </div>
        ...
        {step === "otp" && (
          <div className="flex-1 flex flex-col pt-[52px] pb-8">
            ...
            <h2 className="font-serif text-[28px] leading-[1.05] mb-2">
              Enter the code
            </h2>
            <p className="text-[13px] text-muted leading-[1.55]">
              We sent a 6-digit code to{" "}
              <b className="text-ink">{maskPhone(digits)}</b>.
            </p>
            ...
            <div className="mt-7">
              ...
              />
              {error && (
                <div className="text-[11.5px] text-danger font-semibold mt-2 text-center">
                  {error}
                </div>
              )}
            </div>
        ...
        {step === "pick" && (
          <div className="flex-1 flex flex-col pt-[52px] pb-8">
            ...
            <p className="text-[13px] text-muted leading-[1.55]">
              This number is registered for more than one profile. Pick the one
              you'd like to open.
            </p>
            {error && (
              <div className="text-[11.5px] text-danger font-semibold mt-3">
                {error}
              </div>
            )}
              ...
              {accounts.map((account) => (
                ...
                >
                  ...
                  <span className="flex-1 min-w-0">
                    <span className="block text-[15px] font-bold text-ink truncate">
                      {account.name}
                    </span>
