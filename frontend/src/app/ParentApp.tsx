...
export function ParentApp() {
  ...
  if (screen === "diary") {
    title = "Diary";
    sub = childName.toUpperCase();
  } else if (screen === "calendar") {
    title = "Calendar";
    sub = (user?.school?.name ?? SCHOOL).toUpperCase();
  } else if (screen === "results") {
    title = "Report Card";
    sub = `${childName}${childKlass ? ` · ${childKlass}` : ""}`.toUpperCase();
  } else if (screen === "photos") {
    title = "Moments";
    sub = "SHARED BY THE SCHOOL";
    ...
    sub = `${childName}${childKlass ? ` · ${childKlass}` : ""}`.toUpperCase();
  } else if (screen === "noticeBoard") {
    title = "Notice Board";
    sub = schoolName.toUpperCase();
  } else if (screen === "notice") {
    title = "Notice";
    sub = (activeNotice?.from ?? schoolName).toUpperCase();
  } else if (screen === "notifs") {
    title = "Notifications";
    sub = schoolName.toUpperCase();
  }
  ...
  return (
            ...
            {students.map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  onClick={() => {
                    setSelStudentId(s.id);
                    setPickerOpen(false);
                  }}
                  className={cx(
                    "flex items-center gap-2.5 px-3.5 py-[13px] rounded-[15px] cursor-pointer mb-2 border-[1.5px]",
                ...
                >
                  ...
                  {on && (
                    <span className="text-green">
                      <Glyph d={GLYPH.check} size={18} stroke={2.4} />
                    </span>
                  )}
                </div>
              );
    ...
    >
      ...
      {screen === "attendance" && (
        <AttendanceParent studentId={selStudentId} go={go} />
      )}
      {screen === "leave" &&
        (leaveSent ? (
          <SuccessScreen
            title="Request sent"
            body="The class teacher will see this right away. You'll get a notification once they've acknowledged it."
  ...
  return (
    <div className="flex items-center gap-[11px] py-2.5 border-t border-line first:border-t-0">
      <div
        onClick={onToggle}
        className={cx(
          "w-[21px] h-[21px] rounded-[7px] flex-none grid place-items-center cursor-pointer border-[1.8px]",
          done ? "bg-green border-green text-white" : "bg-white border-line",
        )}
      >
      ...
      <div className="flex-1">
        ...
        <small className="block text-muted text-[11px] mt-px">
          {entry.task}
        </small>
      </div>
    </div>
    ...
    <div className="px-[15px] py-4 pb-6">
      ...
      <div className="flex items-center gap-2 mb-2.5">
        <div className="flex-1 font-serif text-[16px] leading-none text-green">
          ...
          {week[5].getMonth() !== week[0].getMonth() &&
            ` – ${MONTHS[week[5].getMonth()]}`}
          <span className="text-muted text-[13px] font-sans font-semibold ml-1.5">
            {week[0].getFullYear()}
          </span>
        </div>
        {!onCurrentWeek && (
          <button
            onClick={goToday}
            className="text-[11px] font-bold text-green bg-mist px-2.5 py-1.5 rounded-[9px]"
          >
            Today
          </button>
        )}
        <button
          ...
          className="w-7 h-7 rounded-[9px] border border-line bg-white text-muted grid place-items-center"
        >
      ...
      <div className="flex items-center gap-2.5 mb-[11px]">
        <div className="font-serif text-[19px] leading-none">
          {DAY_FULL[selDay.getDay()]}, {selDay.getDate()}{" "}
          {MONTHS[selDay.getMonth()]}
        </div>
        {selDate === todayKey && (
          <span className="text-[10px] font-bold text-green bg-gold-soft px-2 py-[3px] rounded-[7px]">
            TODAY
          </span>
        )}
      </div>
      ...
      {loading && dayEntries.length === 0 ? (
        ...
        <div className="text-center py-10 px-5 text-muted">
          ...
          <p className="text-[12px] leading-[1.5] max-w-[210px] mx-auto">
            Teachers haven't added diary entries for {childName}
            {klassLabel ? ` (${klassLabel})` : ""} on this day yet.
          </p>
        </div>
      ) : (
        <>
          ...
          {notes.map((e) => (
            <Card
              key={e.id}
              className="p-[15px] mb-3 flex gap-[9px] items-start"
            >
              <span className="text-[10px] font-bold text-green bg-mist px-2 py-[3px] rounded-[7px] mt-px flex-none">
                NOTE
              </span>
...
function HomeParent({
  ...
  const todayTasks = diary.filter(
    (e) => e.date === todayKey && e.subject != null,
  );
  ...
  return (
    <div className="px-[15px] pt-4 pb-6">
      ...
      <div
        ...
        className="bg-green text-white rounded-[20px] p-[15px] mb-2.5 flex items-center gap-[13px] cursor-pointer"
      >
        <div className="w-11 h-11 rounded-[14px] bg-white/15 grid place-items-center flex-none">
          <Glyph d={GLYPH.check} size={22} stroke={2.2} />
        </div>
        <div className="flex-1">
          <b className="font-serif text-[20px] font-normal block leading-[1.1]">
            Attendance
          </b>
          <small className="text-[#cfe0d6] text-[11.5px]">
            View this month's register
          </small>
        </div>
      ...
      <button
        onClick={() => go("leave")}
        className="w-full mb-3 py-[11px] rounded-[13px] bg-white border border-line text-green font-semibold text-[12.5px] flex items-center justify-center gap-[7px]"
      >
        <Glyph d={GLYPH.plus} size={16} stroke={1.8} />
        Apply for leave
      </button>
        ...
        {unread.length > 0 ? (
          homeNotices.map((n) => (
            <div
              ...
              className="flex items-center gap-2.5 py-2.5 border-t border-line cursor-pointer"
            >
              ...
              <div className="flex-1 min-w-0">
                <b className="text-[12.5px] font-semibold block leading-[1.25] truncate">
                  {n.title}
                </b>
          ...
          <div className="flex items-center gap-2.5 pt-[11px] pb-1 border-t border-line text-success text-[12.5px] font-semibold">
            <Glyph d={GLYPH.check} size={16} stroke={2.4} />
            You're all caught up on notices
          </div>
        )}
      </Card>
      ...
      <Card onClick={() => go("calendar")} className="p-[15px] mb-3">
        <SectionLabel right={<span className="text-green">Calendar →</span>}>
          Up Next
        </SectionLabel>
        {upNext ? (
          <div className="flex gap-3 items-center">
            <div className="w-[50px] flex-none rounded-[14px] bg-[#f1f5f1] text-center py-2">
              <span className="text-[10px] font-bold text-gold tracking-[0.06em] block">
                {upNext.m}
              </span>
      ...
      <Card onClick={() => go("photos")} className="p-[15px] pb-[13px] mb-3">
        ...
        <div
          className="rounded-[14px] aspect-[2.1] relative overflow-hidden"
          style={{ background: "linear-gradient(150deg,#3d6b4e,#7fae8c)" }}
        >
          <div
            className="absolute inset-0"
            style={{
              background:
                "linear-gradient(180deg,transparent 55%,rgba(0,0,0,.3))",
            }}
          />
          <div className="absolute left-[11px] bottom-[9px] text-white text-[11px] font-semibold z-[2]">
            Annual Sports Day · 142 photos
          </div>
        </div>
      </Card>
    </div>
      ...
      <Card className="p-[15px] mb-3">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-7 gap-1.5 text-center">
            {weekdays.map((w, i) => (
              <div key={i} className="text-[9px] text-muted font-bold">
                {w}
              </div>
            ))}
            {cells.map((d, i) => {
              ...
              return (
                <div
                  key={d}
                  className="aspect-square grid place-items-center text-[11px] font-semibold rounded-[9px] relative"
                  style={{ color: txt }}
                >
                  {d}
                  {st && (
        ...
        <div className="flex gap-3.5 mt-3.5 text-[11px] text-muted font-semibold">
          ...
          <span className="flex items-center gap-1.5">
            <span
              ...
              style={{ background: attColor.LATE }}
            />
            Late
          </span>
        </div>
...
function LeaveParent({
  ...
  return (
    <div className="px-[15px] py-4 pb-6">
      ...
      <div className="mb-3.5">
        ...
        <div className="flex gap-[9px]">
          <input
            ...
            className="w-full px-3 py-[11px] border-[1.5px] border-line rounded-xl text-[13px] bg-white text-center"
          />
          <input
            type="date"
            value={toDate}
      ...
      <div className="mb-3.5">
        <label className="block text-[12px] font-semibold mb-[7px]">
          Reason for the class teacher
        </label>
        <textarea
          value={reason}
          onChange={(e) => setReason(e.target.value)}
          placeholder="e.g. Mild fever, advised rest by the doctor."
          className="w-full box-border px-3 py-[11px] border-[1.5px] border-line rounded-xl text-[13px] bg-white resize-none h-[74px]"
        />
      </div>
      {err && (
        <div className="text-[11.5px] text-danger font-semibold mb-2">
          {err}
        </div>
      )}
...
function PastLeaves({ studentId }: { studentId: number | null }) {
  ...
  const total = thisYear.reduce(
    (n, l) => n + leaveDays(l.fromDate, l.toDate),
    0,
  );
        ...
        return (
          <Card key={l.id} className="p-[13px] mb-2.5">
            <div className="flex items-center gap-2 mb-1.5">
              <span
                className={cx(
                  "text-[9.5px] font-bold px-[7px] py-[3px] rounded-[6px] tracking-[0.04em] flex-none",
                  LEAVE_BADGE[l.type],
                )}
              >
                {l.type}
              ...
              <b className="text-[13px] font-bold">
                {leaveRange(l.fromDate, l.toDate)}
              </b>
              <small className="text-[11px] text-muted">
                {daysLabel(leaveDays(l.fromDate, l.toDate))}
              </small>
              <span
                className={cx(
                  "ml-auto text-[10.5px] font-semibold flex-none",
                  pending ? "text-[#8a6d1f]" : "text-muted",
                )}
              >
...
function ResultsParent({ studentId }: { studentId: number | null }) {
  ...
  if (termsLoading) {
    return (
      <div className="px-[15px] py-10 text-center text-muted text-[12.5px]">
        Loading results…
      </div>
    );
  }
  if (terms.length === 0) {
    return (
      <div className="text-center py-12 px-5 text-muted">
        ...
        <p className="text-[12px] leading-[1.5] max-w-[210px] mx-auto">
          You'll get a notification when the school publishes your child's
          report card.
        </p>
      </div>
    );
  ...
  return (
    <div className="px-[15px] py-4 pb-6">
      ...
      <div className="gw-scroll flex gap-2 overflow-x-auto mb-4 pb-0.5">
        {terms.map((t) => {
          ...
          return (
            <button
              key={t.id}
              onClick={() => setSelTermId(t.id)}
              className={cx(
                "flex-none min-w-[110px] text-left px-[13px] py-2.5 rounded-[14px] border",
      ...
      {resultsLoading ? (
                ...
                <div key={i} className="mb-[13px] last:mb-0">
                  ...
                  <div className="h-[7px] rounded-md bg-[#f1f5f1] overflow-hidden">
                    <i
                      ...
                      style={{
                        width: `${pct}%`,
                        background: "linear-gradient(90deg,#15412f,#1d5740)",
                      }}
                    />
                  </div>
                </div>
        ...
        <div className="text-center py-10 px-5 text-muted text-[12.5px]">
          No subject marks recorded for this exam yet.
        </div>
      )}
    </div>
  );
